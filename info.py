...
def plot(path: str, values: list, title: str, labels: list, dtype: str):
    ...
    colors = ['b', 'r', 'y']
    assert dtype in ['hist', 'plot']

    if dtype == 'plot':
        for y, label, color in zip(values, labels, colors):
            x = numpy.arange(0, len(y), 1)
            axis.plot(x, y, color, alpha=0.5 if len(values) > 1 else 1, label=label)

    elif dtype == 'hist':
        for y, label, color in zip(values, labels, colors):
            x = numpy.arange(0, len(y), 1)
            axis.bar(x, height=y, color=color, alpha=0.5 if len(values) > 1 else 1, label=label)
